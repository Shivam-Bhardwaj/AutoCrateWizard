...
def get_nx_label_html(label_text):
    ...
    # Ensure label_text is escaped if it can contain HTML special characters,
    # though for NX variable names it's usually not an issue.
    # For safety, one might use html.escape(label_text) if label_text could be arbitrary.
...
def input_slider_combo(label_text, min_val, max_val, default_val_from_ss, step, format_str="%.1f", help_text="", key_prefix=""):
    ...
    with slider_col:
        new_slider_widget_val = st.slider(
            label_text, # Use plain text label directly
    ...
    with num_col:
        ...
        new_num_widget_val = st.number_input(
            ...
            min_value=min_val, max_value=max_val, value=st.session_state[session_key_value], 
...
# Session state keys for input_slider_combo are now based on the key_prefix and the sanitized plain text label.
# For st.number_input managed by manage_input_state, the session_value_key is used.
default_inputs_init = {
...
if run_data_calculations:
    ...
    # CORRECTED: Access default_inputs_init with the correct session state keys for slider_combo inputs
    ...
    if _skid_results != st.session_state.skid_results: 
        st.session_state.skid_results = _skid_results; data_actually_changed = True; log.info("Skid results changed.")
    ...
    if _overall_dims_new != st.session_state.overall_dims_for_display: 
        st.session_state.overall_dims_for_display = _overall_dims_new; data_actually_changed = True; log.info("Overall dimensions changed.")
    
    if data_actually_changed or st.session_state.bom_dataframe is None or \
       (isinstance(st.session_state.bom_dataframe, pd.DataFrame) and st.session_state.bom_dataframe.empty and \
        ...
        if not data_actually_changed: st.session_state.visuals_are_stale = True; log.info("BOM recompiled, marking visuals stale.")
...
if show_visuals_toggle:
    if not st.session_state.first_data_run_complete:
        st.info("Calculate data first by adjusting parameters above and clicking '🔄 Regenerate Crate Layout'.")
    else:
        ...
        if needs_generation:
            ...
            with st.spinner("Generating visualization figures..."):
                ...
                if visualizations_available:
                    if can_generate_base:
                        ...
                        st.session_state.fig_base_top, st.session_state.fig_base_front, st.session_state.fig_base_side = None, None, None
            ...
            st.session_state.visuals_are_stale = False; st.session_state.visuals_generated_at_least_once = True
            log.info("Visual generation attempt complete."); st.rerun()

        if not st.session_state.visuals_generated_at_least_once: st.info("Visuals have not been generated.")
        elif st.session_state.visuals_are_stale: st.warning("Visuals are outdated.")
        else: 
            ...
            if _top_res_exp and _overall_dims_exp and LOGIC_IMPORTED and hasattr(explanations, 'get_top_panel_explanation'): 
