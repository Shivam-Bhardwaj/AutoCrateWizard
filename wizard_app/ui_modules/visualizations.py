...
Uses centralized styling, coordinate systems (no grid), and includes variable annotations.
Ensures correct floorboard alignment and side view detail.
"""
...
try:
    # Use absolute imports assuming wizard_app is runnable as a package
    from wizard_app import config
    from wizard_app import explanations
except ImportError:
    # Fallback for local testing or path issues
    try:
        import config
        import explanations
    except ImportError as e:
        # If imports fail here, it's critical. Stop execution.
        logging.error(f"FATAL: Failed to import config/explanations in visualizations.py: {e}", exc_info=True)
        # Attempt to show error in Streamlit if st is defined
        try:
            st.error(f"Code Error: Failed to load critical configuration or text modules. Cannot proceed. Error: {e}")
        except NameError:
            print(f"FATAL Error: Could not import config/explanations: {e}") # Log to console if st fails
        st.stop() # Stop app execution if essential modules are missing
...
def create_schematic_view(title, width_hint, height_hint, components=[], annotations=[],
    ...
    1:1 aspect ratio, visible coordinate system (no grid), dark fonts, and centralized styling.
    """
    fig = go.Figure()
    ...
    for comp in components:
        ...
        # Update min/max considering both endpoints
    ...
    for ann in annotations:
        ...
        fig.add_annotation(
            ...
            font=dict(size=ann.get("size", config.ANNOT_FONT_SIZE_NORMAL), color=ann.get("color", config.COMPONENT_FONT_COLOR_DARK)), # Use dark font default
            align=ann.get("align", "center"), bgcolor=ann.get("bgcolor", "rgba(255,255,255,0)"), # Default transparent BG
        ...
        # Simplified bound update for annotations
    ...
    actual_data_width = max(max_x_data - min_x_data, 1.0) # Ensure non-zero extent
    actual_data_height = max(max_y_data - min_y_data, 1.0)
    # Ensure origin is included if data is near it
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            range=y_range, showgrid=False, # GRID REMOVED
            zeroline=True, zerolinewidth=1, zerolinecolor=config.AXIS_ZERO_LINE_COLOR,
            showticklabels=True, tickfont=dict(size=config.TICK_LABEL_FONT_SIZE, color=config.COMPONENT_FONT_COLOR_DARK),
        ...
        margin=dict(l=60, r=30, t=80, b=60), # Adjusted margins
...
def create_panel_assembly_views(panel_data, panel_label_base="Panel"):
    ...
    if panel_w <= config.FLOAT_TOLERANCE or panel_h <= config.FLOAT_TOLERANCE: error_msg = f"{panel_label_base} dimensions invalid (W:{panel_w:.2f}, H:{panel_h:.2f})."; fig_unavailable = go.Figure(layout=dict(title=f"{panel_label_base} (Invalid Dims)", width=300, height=200)); fig_unavailable.update_xaxes(visible=True, showticklabels=True, title_text="X"); fig_unavailable.update_yaxes(visible=True, showticklabels=True, title_text="Y/Z", scaleanchor="x", scaleratio=1); return fig_unavailable, fig_unavailable, error_msg
    ...
    for cleat in all_cleats_for_front_view:
        ...
        if c_len > 1.0 and c_rect_width > 1.0 : front_annotations.append({"x": abs_center_x, "y": abs_center_y, "text": text_annot, "size": config.ANNOT_FONT_SIZE_SMALL, "color": config.CLEAT_FONT_COLOR, "bgcolor": config.ANNOT_BGCOLOR_DARK}) # White text on dark bg
    ...
    # --- Profile View ---
    profile_components, profile_annotations = [], []; profile_plot_height = plywood_t + cleat_actual_thickness
    ...
    profile_dim_var = height_var_name; thickness_ply_var = "cap_panel_thickness" if is_top_panel else "plywood_thickness"; thickness_cleat_var = "cap_cleat_thickness" if is_top_panel else "wall_cleat_thickness"
    ...
    profile_plane = "ZY" if not is_top_panel else "YZ"; fig_profile = create_schematic_view(f"{panel_label_base} - Profile View ({profile_plane} Plane)", profile_plot_width, profile_plot_height, profile_components, profile_annotations, xlabel=profile_xlabel, ylabel=profile_ylabel)
    return fig_front, fig_profile, None


# --- Base Assembly Views Function ---
def display_base_assembly_views(skid_results, floor_results, wall_results, overall_dims, ui_inputs):
    """
    Displays Top (XY), Front (XZ), and Side (YZ) orthographic views
    of the Crate Base assembly (Skids + Floorboards) side-by-side.
    Includes variable annotations and detailed floorboard side view.
    """
    ...
    if floor_status not in ["OK", "WARNING"]: st.info(f"Base Assembly requires 'OK' or 'WARNING' floorboard status (Current: {floor_status})."); return

    panel_thickness = ui_inputs.get('panel_thickness', config.DEFAULT_PANEL_THICKNESS_UI); wall_cleat_thickness = ui_inputs.get('wall_cleat_thickness', config.DEFAULT_CLEAT_NOMINAL_THICKNESS)
    wall_assembly_offset = panel_thickness + wall_cleat_thickness
    fig_top, fig_front, fig_side = None, None, None # Initialize

    try:
        # Extract dimensions
        skid_w = skid_results.get('skid_width'); skid_h = skid_results.get('skid_height'); skid_x_positions = skid_results.get('skid_positions'); overall_skid_span = overall_dims.get('overall_skid_span')
        floorboard_thick = config.STANDARD_FLOORBOARD_LUMBER_ACTUAL_THICKNESS; floorboard_length = floor_results.get("floorboard_length_across_skids"); floorboard_layout_span = floor_results.get("target_span_along_length"); all_floorboards = floor_results.get("floorboards", []); fb_center_gap_viz = floor_results.get("center_gap", 0.0)
        crate_length = overall_dims.get('length')

        if not all([skid_w, skid_h, skid_x_positions, overall_skid_span, floorboard_thick, floorboard_length, floorboard_layout_span is not None, crate_length]): raise ValueError("Missing critical dimensions")
        ...
        if not math.isclose(calculated_span, overall_skid_span, rel_tol=1e-3): log.warning(f"Base Assy: Span recalc {calculated_span:.3f} != metric {overall_skid_span:.3f}."); overall_skid_span = calculated_span

        # --- Generate Top View (XY) Figure ---
        ...
        fig_top = create_schematic_view( "Top View (XY Plane)", overall_skid_span, crate_length, components_top, annotations_top, xlabel="X (Width) [in]", ylabel="Y (Length) [in]" )

        # --- Generate Front View (XZ) Figure ---
        ...
        fig_front = create_schematic_view( "Front View (XZ Plane)", overall_skid_span, plot_height_hint_front, components_front, annotations_front, xlabel="X (Width) [in]", ylabel="Z (Height) [in]" )

        # --- Generate Side View (YZ) Figure with Detail ---
        ...
        fig_side = create_schematic_view( "Side View (YZ Plane)", crate_length, plot_height_hint_side, components_side, annotations_side, xlabel="Y (Length) [in]", ylabel="Z (Height) [in]" )

        # --- Display side-by-side ---
        col1, col2, col3 = st.columns(3)
        with col1: st.caption("Top View (XY)"); st.plotly_chart(fig_top, use_container_width=True)
        with col2: st.caption("Front View (XZ)"); st.plotly_chart(fig_front, use_container_width=True)
        with col3: st.caption("Side View (YZ)"); st.plotly_chart(fig_side, use_container_width=True)

    except ValueError as ve: st.warning(f"⚠️ Could not generate Base Assembly views: {ve}"); log.error(f"Error in Base Assembly views generation: {ve}")
    except Exception as e: st.error(f"An unexpected error occurred generating Base Assembly views: {e}"); log.error(f"Unexpected error in Base Assembly views generation:", exc_info=True)


# --- Wall Assembly Display Function ---
def display_wall_assembly(wall_panel_data, panel_label_base, ui_inputs, overall_dims):
    ...
    if not wall_panel_data or wall_panel_data.get("panel_width", 0) == 0: st.info(f"{assy_label} data not available or panel has zero width."); return
    try:
...
# --- Top Panel Assembly Display Function ---
def display_top_panel_assembly(top_panel_data, ui_inputs, overall_dims):
    ...
    if not top_panel_data or top_panel_data.get("status") not in ["OK", "WARNING"]: st.info(f"{assy_label} data not available or not valid. Status: {top_panel_data.get('status', 'N/A') if top_panel_data else 'No Data'}"); return
    try:
