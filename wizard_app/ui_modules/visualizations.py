...
MODIFIED: Reverted fixedrange to False to enable zoom/pan and autoscale button.
          Increased padding to reduce text crowding.
...
def create_schematic_view(title_html, width_hint, height_hint, components=[], annotations=[],
    ...
    padding_x = max(actual_data_width * 0.15, 10)  # MODIFICATION: Increased padding
    padding_y = max(actual_data_height * 0.15, 10) # MODIFICATION: Increased padding
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            visible=True,
            fixedrange=False,  # MODIFICATION: Reverted to False
...
def _create_panel_assembly_figure(view_type, panel_data, panel_label_base="Panel"):
    ...
    if view_type == "Front":
        ...
        annotations.append({"x": panel_w / 2.0, "y": - (panel_h * 0.05) - 5, # Adjusted y for padding
                            "text": nx_annot(f'{panel_w:.2f}"', dim_var_w),
                            "size": DEFAULT_ANNOT_FONT_SIZE_NORMAL, "color": NX_ANNOT_FONT_COLOR,
                            "yanchor": "top", "yshift":-5})
        annotations.append({"x": - (panel_w * 0.05) - 5, "y": panel_h / 2.0, # Adjusted x for padding
                            "text": nx_annot(f'{panel_h:.2f}"', dim_var_h),
                            "size": DEFAULT_ANNOT_FONT_SIZE_NORMAL, "color": NX_ANNOT_FONT_COLOR,
                            "textangle": -90, "xanchor": "right", "xshift":-15}) # Ensure xanchor allows space
        ...
        annotations.append({"x": profile_plot_width / 2.0, "y": - (profile_plot_height_assy * 0.05) - 5, # Adjusted y
                            "text": nx_annot(f'{profile_plot_width:.2f}"', dim_var_profile_len),
                            "size": DEFAULT_ANNOT_FONT_SIZE_NORMAL, "color": NX_ANNOT_FONT_COLOR,
                            "yanchor": "top", "yshift":-5})
        annotations.append({"x": - (profile_plot_width * 0.05) - 5, "y": profile_plot_height_assy / 2.0, # Adjusted x
...
def _create_base_top_view_fig(skid_results, floor_results, overall_dims, ui_inputs):
    try:
        ...
        annotations_top.append({"x": 0, "y": - (crate_length * 0.05) -5 , # Adjusted y
        ...
        annotations_top.append({"x": plot_x0_boards - abs(plot_x0_boards*0.05) - 10, "y": crate_length / 2.0, # Adjusted x
...
def _create_base_front_view_fig(skid_results, floor_results, overall_dims):
    try:
        ...
        annotations_front.append({"x": 0, "y": -plot_height_hint_front*0.05 - 5, # Adjusted y
                                  "text": nx_annot(f'Overall Skid Span (X): {overall_skid_span:.2f}"', "VAR_Overall_Skid_Span"),
                                  "size": DEFAULT_ANNOT_FONT_SIZE_NORMAL, "color": NX_ANNOT_FONT_COLOR, "yanchor": "top", "yshift":-5})
        annotations_front.append({"x": fb_x0 - abs(fb_x0*0.05) - 10, "y": total_base_height / 2.0, # Adjusted x
...
def _create_base_side_view_fig(skid_results, floor_results, overall_dims, ui_inputs):
    try:
        ...
        annotations_side.append({"x": crate_length / 2.0, "y": -plot_height_hint_side*0.05 - 5, # Adjusted y
                                 "text": nx_annot(f'Crate Length (Y): {crate_length:.2f}"', "OUT_Crate_Length"),
                                 "size": DEFAULT_ANNOT_FONT_SIZE_NORMAL, "color": NX_ANNOT_FONT_COLOR, "yanchor": "top", "yshift":-5})
        annotations_side.append({"x": -plot_width_hint_side*0.05 - 10, "y": total_base_height_side / 2.0, # Adjusted x
