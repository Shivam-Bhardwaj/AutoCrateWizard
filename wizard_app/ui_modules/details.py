...
Corrected handling of missing numeric data for Arrow compatibility.
"""
import streamlit as st
import pandas as pd
import math # Needed for isnan

def format_for_display(value, precision=2):
    """Helper to format numbers for display, handling None/NaN."""
    if value is None or (isinstance(value, float) and math.isnan(value)):
        return "-" # Display None or NaN as hyphen
    if isinstance(value, (int, float)):
        return f"{value:.{precision}f}"
    return str(value) # Return string representation for non-numbers
...
def display_details_tables(wall_results, floor_results, top_panel_results):
            ...
            if wall_details_data:
                ...
                st.dataframe(
                    ...
                    column_config={
                         ...
                         "Thickness (in)": st.column_config.NumberColumn(format="%.3f"), # More precision for thickness
                         "Center Pos X (rel)": st.column_config.NumberColumn(format="%.2f"),
                         "Center Pos Y (rel)": st.column_config.NumberColumn(format="%.2f"),
                    }
                )
            else: st.caption("No wall cleat details to display.")
    ...
    with st.expander("Floorboard Details", expanded=False):
        if floor_results and floor_results.get("status") in ["OK", "WARNING"] and floor_results.get("floorboards"):
            ...
            st.dataframe(
                df_boards_table, use_container_width=True, hide_index=True,
                column_config={
        ...
        if top_panel_results and top_panel_results.get("status") in ["OK", "WARNING"]:
            ...
            if lc_data.get("count",0)>0:
                for i,pos_x in enumerate(lc_data.get("positions",[])):
                     # Ensure data is numeric or None before adding
                     cap_details_data.append({
                         "Type":"Longitudinal","Cleat #":i+1,
                         "Length (in)": float(lc_data.get("cleat_length_each")) if isinstance(lc_data.get("cleat_length_each"), (int,float)) else None,
                         "Width (in)": float(lc_data.get("cleat_width_each")) if isinstance(lc_data.get("cleat_width_each"), (int,float)) else None,
                         "Thickness (in)": float(lc_data.get("cleat_thickness_each")) if isinstance(lc_data.get("cleat_thickness_each"), (int,float)) else None,
                         "Center Pos X (rel)": float(pos_x) if isinstance(pos_x, (int,float)) else None,
                         "Center Pos Y (rel)": None # Y Pos not applicable here
                     })
            if tc_data.get("count",0)>0:
                for i,pos_y in enumerate(tc_data.get("positions",[])):
                     cap_details_data.append({
                         "Type":"Transverse","Cleat #":i+1,
                         "Length (in)": float(tc_data.get("cleat_length_each")) if isinstance(tc_data.get("cleat_length_each"), (int,float)) else None,
                         "Width (in)": float(tc_data.get("cleat_width_each")) if isinstance(tc_data.get("cleat_width_each"), (int,float)) else None,
                         "Thickness (in)": float(tc_data.get("cleat_thickness_each")) if isinstance(tc_data.get("cleat_thickness_each"), (int,float)) else None,
                         "Center Pos X (rel)": None, # X Pos not applicable here
                         "Center Pos Y (rel)": float(pos_y) if isinstance(pos_y, (int,float)) else None
                     })
            if cap_details_data:
                df_cap_details = pd.DataFrame(cap_details_data);
                st.dataframe(
                    df_cap_details,use_container_width=True,hide_index=True,
                    column_config={
                         "Length (in)":st.column_config.NumberColumn(format="%.2f"),
                         "Width (in)":st.column_config.NumberColumn(format="%.2f"),
            ...
            else: st.caption("No top panel cleat details to display.")
